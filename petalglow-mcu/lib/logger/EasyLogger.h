...
#define _EASYLOGGGER_h
    ...
    // Default log formatting is LOG_FORMATTING_HMS, if it's not defined in user code.
        ...
        // Prints time, loglevel and service. No _EASYLOGGGER_ENDLine. That will be streamed by macro-wrapper
    ...
    /* Reusable code for each loglevel macro function. If filtering is enabled this will be
       ...
       It will log time-stamp and log-content with an _EASYLOGGGER_ENDLine */
    ...
    {                                                       \
        ...
        {                                                   \
            ...
            LOG_OUTPUT << content;                          \
            LOG_OUTPUT << _EASYLOGGGER_ENDL;                             \
        }                                                   \
    }

    /* Reusable code for each loglevel macro funcion. This code is inserted in the log-statement
       if filtering is disabled. Just prints a time-stamp and the log content with an _EASYLOGGGER_ENDLine */
